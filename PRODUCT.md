...
- [x] Create and securely back up the persistent private POC keystore on the trusted build machine.
- [x] Pin the neutral VK-facing Android POC identity selected for later registration to `app.northbridge.mobile`, the `Northbridge` launcher label and the neutral persistent certificate subject.
- [x] Require the first identity bootstrap to match an operator-approved certificate SHA-256 and derive its application ID from both accepted APK manifests.
- [x] Verify the merged signed APK has exactly one expected launcher, no legacy app/VPN components, `allowBackup=false` and `usesCleartextTraffic=false`.
